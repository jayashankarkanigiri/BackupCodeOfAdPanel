  ...
  //       // stay type
  //       formData.append("stay_type", form.stayType);
  ...
  //           type: form.documents.property.mimeType || "application/octet-stream",
  //         });
  //       }
  ...
  //         "http://192.168.1.37:8000/api/owner/",
  //         {
  //           method: "POST",
  //           body: formData,
  //         }
...
export default function OwnerCommercialSection() {
  ...
  const pickDoc = async (key) => {
    ...
    const res = await DocumentPicker.getDocumentAsync({
      multiple: isMultiple,
      type: ["image/*", "application/pdf"], // allow only images and pdf
    });
    ...
    if (!res.canceled && res.assets && res.assets.length > 0) {
      ...
      if (isMultiple) {
        ...
        res.assets.forEach((asset) => {
          ...
          if (!isImage && !isPDF) {
            hasSizeError = true;
            newErrors[`document_${key}`] =
              "Only PDF or image formats are allowed";
            return;
          }

          // image size limit 100KB
          if (isImage && asset.size > 102400) {
            hasSizeError = true;
            newErrors[`document_${key}`] =
  ...
  const handleSubmit = async () => {
    ...
    const submitData = {
      ...
      bankName: form.bankName,
      ifsc: form.ifsc,
      bankNo: form.accountNo, // Adjusted for typical backend naming if needed, but let's stick to what's there and add property_type
      accountNo: form.accountNo,

      floors_info: JSON.stringify(form.floorsData || []),
      property_type: form.stayType, // Added this field
    };
      ...
      },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            //     {
            //       text: "OK",
            //       onPress: () => navigation.replace("OwnerWaitingScreen"),
            //     },
            //   ]);

            // }
            if (response.ok) {
  navigation.replace("OwnerWaitingScreen");
} else {
              Alert.alert("Error", "Registration failed");
            }
          } catch (error) {
                      ...
                      <View style={styles.inputContainer}>
                        ...
                        <TextInput
                          ...
                          onChangeText={(v) => {
                            ...
                            const error = validateConfirmPassword(v, form.password);
                        ...
                        onPress={() => {
                          ...
                          if (form.stayType === "commercial") {
                            ...
                            if (totalSections < 1) {
                              Alert.alert(
                                "Error",
                                "Please add at least 1 section in floor details",
                              );
                              return;
                            }
                            ...
                            for (const floor of floors) {
                              for (const section of floor.sections || []) {
                                if (!section.area) {
                                  Alert.alert(
                                    "Error",
                                    `Enter area (sq.ft) for Floor ${floor.floorNo} Section ${section.sectionNo}`,
                                  );
                                  return;
                                }
                              }
          ...
          >
            ...
            {selectionMode ? (
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                ...
                >
                  <Text style={step3Styles.btnText}>+ Add Floor</Text>
                </TouchableOpacity>
...
function CommercialLayout({ onUpdateFloors }) {
  ...
  const addSectionManually = () => {
    ...
    if (currentSections.length >= 20) {
      Alert.alert("Limit", "Sections per floor cannot exceed 20");
      return;
    }
  ...
  const applyBatchArea = () => {
    Alert.alert(
      "Not Available",
      "Area must be applied inside sections, not floors."
    );
    setAreaBatchModalOpen(false);
  };
          ...
          >
            ...
            {selectionMode ? (
              <View style={step3Styles.selectionFooter}>
                ...
                >
                  <Text style={step3Styles.btnText}>Apply Sections</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View style={{ flexDirection: "row", gap: 10 }}>
                ...
                <ScrollView contentContainerStyle={step3Styles.gridContainer}>
                  {floors[selectedFloor].sections?.map((section, index) => (
                    ...
                    >
                      ...
                      <Text style={step3Styles.cardSub}>
                        {section.area ? `${section.area} sq.ft` : "No area"}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
            ...
            {sectionBatchModalOpen && (
              <View style={step3Styles.batchPopup}>
                <Text style={step3Styles.popupTitle}>
                  Apply Sections to {selectedFloors.length} Floors
                </Text>
            ...
            {areaPopupOpen && selectedFloor !== null && selectedSection !== null && (
              <View style={step3Styles.batchPopup}>
                <Text style={step3Styles.popupTitle}>
                  Enter Area for Section {selectedSection + 1}
                </Text>
